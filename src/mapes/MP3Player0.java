...
/**
 * A sample java program for playing mp3 file
 * Description: 
 * It uses JFilechooser to allow user to choose an MP3 file from disk
 * and then creates a thread to play the chosen mp3 file in background. 
 * 
 * @author wjw
 * 
 * modified from the original source code made by Kevin Wayne
 * source: http://introcs.cs.princeton.edu/java/faq/mp3/mp3.html
 * 
 * Note: 
 * 1. Use it in Netbeans project: 
 * (1) add this java class file into the package in your project
 * (2) you also need to add jLayer.jar, available from MA23's blackboard, 
 *       onto the Libraries of your Java Project.
 * 
 * 
 * 2. If you want to run it standalone, use command below in DOS
 * (jLayer.jar should be in the same folder as this java program.) 
 *  To compile: javac -classpath .;jLayer.jar MP3Player0.java
 *  To run:     java -classpath .;jLayer.jar MP3Player0
 * 
 * Hint: you can use the methods: play() and close() in this java class by
 *       (1) adding it onto your package 
 *       (2) creating instance of it and 
 *       (3) then use the instance to access its methods 
 * 
 * date: 22/12/2014
 * version: v0
 * 
